...
#    items = []
#    if write_header:
#        fields = [ '"' + key + '"' for key in jsrecord.keys() if key != 'attributes' ]
...
def download(job, pool_time=5):
    ...
    while not done:
        try:
            ...
            numberBatchesInProgress = int(job_status['numberBatchesInProgress'])
            ...
            print("{total} batch: {queued} Queued, {inprogress} In Progress, {completed} Completed, {failed} Failed".format(
    ...
    for batch in bulk.get_batch_list(job):
        ...
        if batch['state'] == 'NotProcessed':
            print('Skipping batch ', batch_id, 'in state "NotProcessed".', file=sys.stderr)
            continue
        filename = JOB_DIR + '/' + job + '/' + batch_id + '.' + job_status['contentType']
        with open(filename, 'w') as file:
            print('Downloading batch', batch_id, end='', file=sys.stderr, flush=True)
